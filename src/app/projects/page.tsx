    ...
    <div className="min-h-screen pb-14 pt-10 text-slate-100">
      ...
      <section className="mt-16">
        <div className="space-y-12">
          {featuredProjects.map((project) => (
            <article
              key={project.title}
              className="grid gap-6 border-t border-slate-700/70 pt-8 md:grid-cols-[220px_minmax(0,1fr)]"
            >
              ...
              <div className="max-w-3xl">
                <h2 className="text-3xl font-display text-slate-100">{project.title}</h2>
                <p className="mt-4 text-base text-slate-300">{project.summary}</p>
                <p className="mt-3 text-base text-slate-400">{project.detail}</p>
                <Link
                  href={project.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-6 inline-flex text-sm font-semibold text-slate-200 underline decoration-slate-500 underline-offset-4 transition hover:text-slate-50"
                >
                  Open repository
                </Link>
              </div>
      ...
      <section className="mt-20">
        <div className="max-w-4xl">
          <h2 className="text-4xl font-display text-slate-100">More explorations</h2>
        </div>
        <div className="mt-8 grid gap-10 xl:grid-cols-2">
          {projectArchive.map((project) => (
            <article key={project.title} className="border-t border-slate-700/60 pt-6">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">{project.category}</p>
              <h3 className="mt-3 text-2xl font-display text-slate-100">{project.title}</h3>
              <p className="mt-4 text-base text-slate-300">{project.summary}</p>
              <p className="mt-3 text-base text-slate-400">{project.detail}</p>
